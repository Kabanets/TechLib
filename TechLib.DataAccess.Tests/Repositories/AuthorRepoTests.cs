...
namespace TechLib.DataAccess.Tests.Repositories
{
    ...
    public class AuthorRepoTests
    {
        ...
        {
            var author = authorRepo.Authors.AsQueryable().FirstOrDefault(a => a.FName == "FName1");
        ...
        {
            var author = authorRepo.Authors.AsQueryable().FirstOrDefault(a => a.FName == "FName2");
            Assert.IsNotNull(author);

            authorRepo.DeleteAuthor(author.AuthorId);

            author = authorRepo.Authors.AsQueryable().FirstOrDefault(a => a.FName == "FName2");
